...
The parameter schema defines the type, description, whether it's required, default values, and for select parameters, the available options. The `vaultSecret` type integrates with AgenticVault Security Graph's encrypted credential storage.
...
Always use the Logger utility for tracking progress and errors. The logger integrates with AgenticVault Security Graph's monitoring system and helps users understand what's happening during long-running enrichers. Log successful operations at the info level and errors at the error level.
...
When working with tools, remember that they return raw data structures. Your enricher is responsible for converting tool output into proper AgenticVault Security Graph types. This type conversion is typically done in the scan phase.
...
Remember that enrichers are the heart of AgenticVault Security Graph's intelligence gathering capabilities. Well-designed enrichers that handle errors gracefully, log progress clearly, and create meaningful graph relationships make the entire platform more powerful and user-friendly.
