...
description: "This guide walks you through the process of creating a new data type in the AgenticVault Security Graph ecosystem and integrating it throughout the platform. Types in AgenticVault Security Graph serve as the foundation for all data modeling, providing structure, validation, and schema generation for the entire system."
category: "Developers"
order: 8
author: "AgenticVault Security Graph Team"
...
Currently, AgenticVault Security Graph includes 39 built-in types covering everything from network entities like domains and IPs to identity information like individuals and organizations, security data like credentials and breaches, and financial information like bank accounts and crypto wallets.

### What is FlowsintType?

`FlowsintType` is the base class for all AgenticVault Security Graph entity types. It extends Pydantic's `BaseModel` with additional functionality specific to AgenticVault Security Graph's graph database and UI needs:

```python
class FlowsintType(BaseModel):
    """Base class for all AgenticVault Security Graph entity types with nodeLabel support.
...
AgenticVault Security Graph follows strict naming conventions to maintain consistency across the codebase. Class names use PascalCase (like `Vehicle`, `SocialAccount`, or `CryptoWallet`). Field names use snake_case (like `license_plate`, `phone_number`, or `email_address`). This matches python's standard conventions and makes the codebase more readable.

### Understanding primary fields and labels

Two concepts are crucial for every AgenticVault Security Graph type: the **primary field** and the **nodeLabel**. Understanding these will help you create types that work seamlessly with the graph database and UI.
...
Types often need to reference other AgenticVault Security Graph types. You can import and use them just like any other python type:
...
AgenticVault Security Graph currently organizes types into these standard categories:
...
When referencing other AgenticVault Security Graph types, use relative imports to avoid circular import issues:
...
When your type contains other AgenticVault Security Graph types, test the relationships:
...
- **Nested object issues**: When passing nested AgenticVault Security Graph types, pass the complete object, don't recreate it
...
- **Custom types via API**: AgenticVault Security Graph supports runtime type creation using JSON Schema (see `flowsint-core/src/flowsint_core/core/models.py`)
...
Remember that types are the foundation of everything in AgenticVault Security Graph:
...
With these concepts mastered, you're ready to create powerful, robust types that will make the entire AgenticVault Security Graph platform more effective for intelligence gathering.
