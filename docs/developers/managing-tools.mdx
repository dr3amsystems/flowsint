...
description: "This guide explains how to create a new tool in the AgenticVault Security Graph ecosystem. Tools are low-level wrappers around external utilities, Docker containers, and APIs that enrichers use to gather intelligence. Understanding the tool architecture will help you extend AgenticVault Security Graph's capabilities with new data sources and reconnaissance utilities."
category: "Developers"
order: 9
...
Tools in AgenticVault Security Graph serve as abstraction layers between enrichers and external systems. They provide a consistent interface for executing Docker containers, calling APIs, or running python libraries. While enrichers handle high-level orchestration and graph database operations, tools focus exclusively on executing external commands and returning raw results.

Every tool implements a basic interface with methods for naming, categorization, and execution. Tools don't know anything about Pydantic types, Neo4j graphs, or the broader AgenticVault Security Graph architecture. They just wrap external functionality and return data.

AgenticVault Security Graph currently includes tools for subdomain enumeration, port scanning, DNS queries, WHOIS lookups, web crawling, and business intelligence.
...
Notice how the tool returns simple python data structures like lists and dictionaries. Tools don't know about Pydantic types or AgenticVault Security Graph models. That's the enricher's job.

## Creating a docker-based tool

Docker-based tools are more common in AgenticVault Security Graph because most reconnaissance utilities need specific dependencies and isolated environments. Let's walk through creating a tool that wraps a hypothetical Docker-based subdomain scanner.
...
The enricher instantiates your tool, calls its `launch` method with appropriate parameters, and processes the results into AgenticVault Security Graph types.

## Common patterns

Several patterns appear frequently in AgenticVault Security Graph tools. Understanding these will help you write tools that fit naturally into the ecosystem.
...
Once you've created your tool and tested it, you can build enrichers that use it. Enrichers orchestrate one or more tools to gather intelligence, validate the results, convert them to AgenticVault Security Graph types, and create graph database nodes and relationships.
...
Remember that tools are just one layer in the AgenticVault Security Graph architecture. They provide the raw capabilities, but enrichers provide the intelligence and graph-building logic that makes the platform powerful.
