...
A good amount of the tools you'll be using in AgenticVault Security Graph require third party API keys.

The **Vault** (*"Coffre fort"* in french) is the place to centralize and securely store those API keys. 
Weither you have a local instance of AgenticVault Security Graph or one fully deployed on a distributed system, you need to have your keys securely stored.

## Adding a key

In the AgenticVault Security Graph enricher ecosystem, the API keys follow a specific format, being in uppercase letters, and with a declarative name that follows `<service>_API_KEY`.
...
In the meanwhile, here is a list of the needed keys to run AgenticVault Security Graph at it's full potential:
